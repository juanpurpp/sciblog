...
const Fotoforo = ({ className, nombre, description, idT, correo, usuarioCorreo, onDeleteSuccess }) => {
  const forMutation = useMutation({
    mutationFn: (data) => borrarTema(data),
    onSuccess: () => {
      console.log('Tema eliminado correctamente');
      onDeleteSuccess(); // Ejecutar callback al eliminar el tema con éxito
    },
    onError: (error) => {
